...
- Content Studio now includes a guarded release manager: it lists current course versions and blockers, exposes lesson-version diffs, and permits audited preview/publish/withdraw decisions only after the source and independent-review gates pass.
...
The following design item remains explicit follow-on work rather than hidden behavior: richer release-history/version-diff presentation beyond the current guarded release manager. DOCX/PPTX XML extraction, URL/YouTube import, Google Vision OCR, resumable chunk sessions, extraction diagnostics, stale-job recovery, session-isolated worker heartbeats during model calls, block-level passage IDs, team-scoped parent consent, configurable fail-closed ClamAV scanning on Cloud Run, and guarded preview/publish/withdraw decisions are now supported. Production revision `soplat-web-00073-pvx` bundles ClamAV and refreshes its signature database during the image build; uploads fail closed when the scanner is unavailable.
