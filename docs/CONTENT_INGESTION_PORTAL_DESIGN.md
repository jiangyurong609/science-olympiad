...
The following design item remains explicit follow-on work rather than hidden behavior: a richer visual release-history/version-diff workspace beyond the current guarded API and release-manager controls. DOCX/PPTX XML extraction, URL/YouTube import, Google Vision OCR, resumable chunk sessions, extraction diagnostics, stale-job recovery, session-isolated worker heartbeats during model calls, block-level passage IDs, team-scoped parent consent, durable GCS artifacts, configurable fail-closed ClamAV scanning on Cloud Run, and guarded preview/publish/withdraw decisions are now supported. Production revision `soplat-web-00074-p97` bundles ClamAV, refreshes its signature database during the image build, and uses the regional GCS artifact bucket; uploads fail closed when the scanner is unavailable.
