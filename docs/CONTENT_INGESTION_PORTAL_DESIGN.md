...
- Staff have a split-pane source inspector with immutable snapshot metadata and passage IDs; parent/student relationships are explicit pending/approved/rejected records, and material assignment requires an approved relationship. A relationship can optionally be scoped to a team, with both parent and student team membership verified before staff approval or assignment.
...
- Production deploys use the regional `soplat-artifacts-video-agent-493605` GCS bucket with generated content-hash keys; a live storage round-trip has been verified, so Cloud Run restarts do not discard intake artifacts.
...
- Content Studio now includes a guarded release manager: it lists current course versions and blockers, exposes lesson-version diffs and release-transition history, and permits audited preview/publish/withdraw decisions only after the source and independent-review gates pass.
...
- Real external-video validation has been run against Science Olympiad videos (`i22uB-vXXS8` and `7DLU4tfEIVY`): caption retrieval produced 620 and 766 timestamped segments. A real authenticated production upload of the first transcript completed Cloud Run scanning, GCS storage, extraction, source inspection (7 timestamped passages), and explicit rejection, so it never became student-visible.
- Lesson authoring now composes a grounded media layer after text generation: approved event videos are embedded with transcript passage IDs, and licensed event image-manifest assets become an observation gallery with attribution and alt text. The Content Studio `Add grounded media` action creates a new editor-review version for already published lessons, so multimedia can be rolled out without silently changing graded student content.

DOCX/PPTX XML extraction, URL/YouTube import, Google Vision OCR, resumable chunk sessions, extraction diagnostics, stale-job recovery, session-isolated worker heartbeats during model calls, block-level passage IDs, team-scoped parent consent, durable GCS artifacts, configurable fail-closed ClamAV scanning on Cloud Run, guarded preview/publish/withdraw decisions, release-transition history, and the visual release-history workspace are now supported. Production revision `soplat-web-00077-hn8` bundles ClamAV, refreshes its signature database during the image build, uses the regional GCS artifact bucket, and allocates 2 GiB with single-request concurrency for scanner safety; uploads fail closed when the scanner is unavailable.
